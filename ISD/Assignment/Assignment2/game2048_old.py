...
class Game2048Env(gym.Env):
    ...
    def run(self):
        ...
        port, remote_port = Pipe(duplex=True)  #Pipe(True)返回两个连接对象，分别代表父进程和子进程
        self.port = port
...
class Game2048GUI(QDialog):
    ...
    def _mainLoop(self):  #up:0,down:1,left:2,right:3
        while True:
...
if __name__ == '__main__':
    env = Game2048Env(True, True, True)    
    obs = env.reset()
    done = False
    while not done:
